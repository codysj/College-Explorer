...
class School(Base):
    ...
    embeddings: Mapped[list[SchoolEmbedding]] = relationship(back_populates="school")
...
class SchoolEmbedding(Base):
    __tablename__ = "school_embeddings"

    school_id: Mapped[int] = mapped_column(ForeignKey("schools.id", ondelete="CASCADE"), primary_key=True)
    embedding_type: Mapped[str] = mapped_column(String(40), primary_key=True)
    embedding_model: Mapped[str] = mapped_column(String(80), primary_key=True)
    vector: Mapped[str] = mapped_column(Text, nullable=False)
    text_snapshot_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    refreshed_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())

    school: Mapped[School] = relationship(back_populates="embeddings")
